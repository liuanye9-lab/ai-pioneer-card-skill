...
- End-to-end verification fix: lark-cli CardKit calls now force bot identity because user access tokens are rejected.
- End-to-end verification fix: generated bytes are staged under the working tree and uploaded through a relative path because lark-cli rejects system-temp and absolute file paths on macOS.
