...
export class FeishuCardAdapter {
  ...
  async uploadImageBytesViaCli(bytes: Uint8Array, name = "generated.png"): Promise<UploadImageResult> {
    // lark-cli restricts uploads to its working tree on macOS, so stage the
    // generated bytes under cwd instead of the system temp directory.
    const dir = await mkdtemp(join(process.cwd(), ".aipc-image-"));
    ...
    try {
      ...
      const result = await runLarkCli([
        ...
        "--file",
        `image=./${relative(process.cwd(), file)}`,
        "--json",
      ]);
